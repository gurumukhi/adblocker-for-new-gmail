var SCRIPT_ADDITION_ATTEMP_COUNT = 10,
...
var scriptAdditionAttemptsCount = 0;

var attemptScriptAddtion = () => {
  if(addCleanAdScript()) {
...
var addCleanAdScript = () => {
  cleanAds();
  var tabs = document.querySelector(TAB_SELECTOR);
...
var attempAdCleanMultipleTimes = (attemptCount, attemptTimeout) => {
...
var cleanAds = () => {
  try {
    var adRow = document.querySelector(AD_ROW_SELECTOR);
    if (!adRow) {
      return false; // Ad not found or not yet loaded
    }

    var adDiv = adRow.closest('table').closest('div').closest('div');
